// The task involves creating a loader in C that loads and executes an ELF 32-bit executable without using any pre-built libraries. The loader should be 
// compiled into a shared library (lib_simpleloader.so) and should be used in conjunction with a helper program to load and execute the target executable.
...
//2. Loader Implementation (loader.c):
...
//3. Loader Shared Library (lib_simpleloader.so):
